    ...
    <footer className="bg-gray-900 text-white">
      <div className="container mx-auto px-4 py-12 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
          ...
          <div className="lg:col-span-1">
            {logo?.image?.asset?._ref ? (
              <SanityImage image={logo} width={150} height={50} className="h-10 w-auto mb-4" />
            ) : (
              <span className="text-xl font-bold mb-4 block">{siteName || 'Pavicat'}</span>
            )}
            {footerText && (
          ...
          {contactInfo && (
            <div>
              ...
              <ul className="space-y-2 text-sm text-gray-400">
                ...
                {contactInfo.piva && <li>P.IVA: {contactInfo.piva}</li>}
              </ul>
            </div>
          )}
        </div>
        ...
        <div className="border-t border-gray-800 mt-10 pt-8 text-center text-sm text-gray-500">
          {copyrightText || `© ${new Date().getFullYear()} Pavicat. Tutti i diritti riservati.`}
        </div>
      </div>
    </footer>
  )
